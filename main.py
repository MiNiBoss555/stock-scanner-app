...
def repair_thai_text(value: str) -> str:
    ...
    def _cp1252_to_bytes_lossless(text: str) -> bytes:
        # Map common mojibake chars to their original byte values.
        out = bytearray()
        for ch in text:
            code = ord(ch)
            if code <= 0xFF:
    ...
    for _ in range(3):
        ...
        try:
            ...
            pass
        try:
            raw_bytes_cp1252 = _cp1252_to_bytes_lossless(repaired)
            converted = raw_bytes_cp1252.decode("utf-8", errors="replace").strip()
            if converted:
                candidates.append(converted)
        except Exception:
            pass
